...
class de_ConvH(nn.Module):
    def __init__(self,image_size=64, inplanes=256, input_channel=50):
        super( de_ConvH, self ).__init__()
        self.pad = 4
        image_size = 64
        self.pad_add =(8-(2 * self.pad + image_size) % 8)%8
        self.convh = nn.Conv2d(inplanes, input_channel, kernel_size=1, stride=1, padding=0,
                               bias=False)
        init.xavier_normal_(self.convh.weight)
        
    def forward(self, feature_c):
        output_h = self.convh(feature_c)        
        out =  output_h[:, :, self.pad:-(self.pad+self.pad_add), self.pad:-(self.pad+self.pad_add)]
        return out
...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor, data: Tensor) -> Tensor:

        feature_a = self.layer1(x)  # 512*8*8->256*16*16
        feature_b = self.layer2(feature_a)  # 256*16*16->128*32*32
        feature_c = self.layer3(feature_b)  # 128*32*32->64*64*64

        feature_c_cut = feature_c[:, :, self.pad:-(self.pad+self.pad_add), self.pad:-(self.pad+self.pad_add)]

        return feature_c, [feature_c, feature_b, feature_a], feature_c_cut

    def forward(self, x: Tensor, data: Tensor) -> Tensor:
