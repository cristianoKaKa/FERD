...
class ConvH(nn.Module):
    def __init__(self,image_size=64, inplanes=64, input_channel=50):
        super( ConvH, self ).__init__()
        self.pad = 4
        self.pad_add =(8-(2 * self.pad + image_size) % 8)%8
        self.convh = nn.Conv2d(input_channel, inplanes, kernel_size=1, stride=1, padding=0,
                               bias=False)
        init.xavier_normal_(self.convh.weight)
...
class Pixel_Classifier(nn.Module):
    def __init__(self,image_size=64, inplanes=256, input_channel=50):
        ...
        self.conv1 = nn.Conv2d(inplanes, h_channels, kernel_size=1, stride=1, padding=0,
                               bias=True)
        self.conv2 = nn.Conv2d(h_channels, input_channel, kernel_size=1, stride=1, padding=0,
                               bias=True)
        self.conv3 = nn.Conv2d(input_channel, 1, kernel_size=1, stride=1, padding=0,
                               bias=True)
    ...
    def forward(self, x1, x2):
        ...
        if torch.abs(x1-x2).mean() != 0:
            f12 = self.conv1(x2)
            f12 = self.relu1(f12)
            f22 = self.conv2(f12)
            f22 = self.relu2(f22)

            out = self.conv3(torch.abs(f22-f21))
...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        feature_a_cut = feature_a[:, :, self.pad:-(self.pad+self.pad_add), self.pad:-(self.pad+self.pad_add)]
        return [feature_a, feature_b, feature_c], feature_a_cut
